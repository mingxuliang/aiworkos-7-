# -*- coding: utf-8 -*-
"""AiWork Agent - Main agent implementation.

This module provides the main AiWorkAgent class built on ReActAgent,
...
class AiWorkAgent(ToolGuardMixin, ReActAgent):
    """AiWork Agent with integrated tools, skills, and memory management.
    ...
    Python's MRO: AiWorkAgent → ToolGuardMixin → ReActAgent.  If you
    ...
    **must** call ``super()._acting(...)`` / ``super()._reasoning(...)``
    ...
    def __init__(
        ...
        user_working_dir: Path | None = None,
    ):
        """Initialize AiWorkAgent.
        ...
        Args:
            ...
            namesake_strategy: Strategy to handle namesake tool functions.
                ...
                (default: "skip")
            workspace_dir: Agent root directory for reading prompt files
                (SOUL.md, AGENTS.md, skills). If None, uses global WORKING_DIR.
            task_tracker: Task tracker for background tasks
            plan_notebook: Plan notebook for structured planning
            user_working_dir: Per-user working directory for file I/O isolation.
                When set, all tool file operations resolve relative to this
                directory instead of workspace_dir. Used for shared agent
                user isolation.
        """
        ...
        self._user_working_dir = user_working_dir
    ...
    def _fix_stringified_json_args(tool_call) -> None:
        ...
        if not isinstance(inp, dict):
            return
        for key in AiWorkAgent._PLAN_JSON_KEYS:
            val = inp.get(key)
    ...
    def _strip_tool_use_from_msg(msg: Msg) -> Msg:
        ...
        if isinstance(msg.content, str):
            msg.content += AiWorkAgent._ROUND_END_NOTICE
            return msg
        ...
        filtered.append(
            {"type": "text", "text": AiWorkAgent._ROUND_END_NOTICE},
        )
        msg.content = filtered
        return msg
    ...
    async def reply(
        ...
        # Use per-user working dir for tool file I/O isolation,
        # fallback to agent root dir for non-shared agents
        set_current_workspace_dir(
            self._user_working_dir or self._workspace_dir,
        )
        ...
        logger.info("AiWorkAgent.reply: max_iters=%s", self.max_iters)
