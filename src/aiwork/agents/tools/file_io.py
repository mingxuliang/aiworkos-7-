...
def _write_file_sync(file_path: str, content: str, encoding: str) -> None:
    """Synchronous file write — offloaded to a thread via asyncio.to_thread."""
...
def _append_file_sync(file_path: str, content: str, encoding: str) -> None:
    """Synchronous file append — offloaded to a thread via asyncio.to_thread."""
...
#    file_path = _resolve_file_path(file_path)
    ...
    try:
        await asyncio.to_thread(_append_file_sync, file_path, content, encoding)
        return ToolResponse(
            content=[
                TextBlock(
                    type="text",
