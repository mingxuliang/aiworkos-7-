...
from ...llm_output.service import upload_and_record
...
async def send_file_to_user(
    ...
    # Attempt MinIO upload; fall back to local file:// URL on failure.
    # This is a fail-open strategy: the user always gets a working URL,
    # regardless of MinIO / DB availability.
    # Lazy-import context getters to avoid circular import at module level
    # (send_file → agent_context → multi_agent_manager → … → send_file).
    ...
    minio_url = await upload_and_record(
        file_path,
        user_id=user_id,
