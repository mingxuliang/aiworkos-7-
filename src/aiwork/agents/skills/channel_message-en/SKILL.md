...
description: Use this skill to proactively send a one-way message to a user/session/channel, usually only when the user explicitly asks to send to a channel/session or when proactive notification is needed. First query sessions with aiwork chats list, then push with aiwork channels send.
metadata:
...
### Should Not Use
- If you are simply replying in the current session, **do not use `aiwork channels send`**
...
`aiwork channels send` requires all of the following:
- `--agent-id`
- `--channel`
- `--target-user`
- `--target-session`
...
`aiwork channels send` only sends — it does not wait for a reply.
...
If you are replying to the user in the current session, do not use `aiwork channels send`.
...
**Selection principle**:
- Need to collaborate with another agent → `aiwork agents chat`
- Need to proactively push a message to a user → `aiwork channels send`
...
- `--target-user`: target user ID (obtained from `aiwork chats list`)
- `--target-session`: target session ID (obtained from `aiwork chats list`)
