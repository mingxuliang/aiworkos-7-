...
def _validate_password(password: str) -> None:
    """Validate that password does not contain any whitespace characters.

    Raises:
        ValueError: If password contains whitespace.
    """
...
async def import_users_from_excel(
    ...
    Returns:
        Tuple of (created_count, error_messages, warning_messages).
    """
    import io
    ...
    for idx, row in enumerate(rows, start=2):
        ...
        # Read raw values (convert to str, default to empty string)
        raw_username = str(row[0]) if row[0] else ""
        raw_password = str(row[1]) if row[1] else ""
        raw_role = str(row[2]) if len(row) > 2 and row[2] else "user"

        # Strip leading/trailing whitespace from username and role
        username_val = raw_username.strip()
        role_val = raw_role.strip()

        # Password: reject if it contains any whitespace
        password_val = raw_password.strip()
        if raw_password != password_val or any(c.isspace() for c in raw_password):
            errors.append(f"Row {idx}: password must not contain spaces")
            continue

        # Detect and warn about whitespace that was automatically trimmed
        if raw_username != username_val:
            all_warnings.append(
                f"Row {idx}: username had leading/trailing spaces (auto-trimmed)"
            )
        if raw_role != role_val:
            all_warnings.append(
                f"Row {idx}: role had leading/trailing spaces (auto-trimmed)"
            )
    ...
    wb.close()
    logger.info(
        "Excel import: created=%d, errors=%d, warnings=%d",
        created, len(errors), len(all_warnings),
    )
    return created, errors, all_warnings
