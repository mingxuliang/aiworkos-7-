...
async def create_chat_service(ws: "Workspace", service):
    ...
    from ..runner.repo.multi_user_repo import MultiUserChatRepository
    ...
    if service is not None:
        ...
        # Create new ChatManager with per-user isolation
        chat_repo = MultiUserChatRepository(ws.workspace_dir)
        cm = ChatManager(repo=chat_repo)
        ws._service_manager.services["chat_manager"] = cm
        logger.info(
            f"ChatManager created (multi-user): {ws.workspace_dir}",
        )
...
async def create_channel_service(ws: "Workspace", _):
    ...
    # Third-party IM channels (WeCom, DingTalk, Feishu, etc.) are managed
    # exclusively by per-user CMs — each JWT-authenticated user gets their
    # own CM with their own credentials and working-directory isolation.
    from ...config.config import ChannelConfig as ChannelConfigModel

    channels_config = ChannelConfigModel()
    agent_console = getattr(ws._config.channels, "console", None)
    if agent_console is not None:
        channels_config.console = agent_console

    temp_config = Config(channels=channels_config)
    ...
    # Discover and start per-user ChannelManagers so that user-owned
    # bot connections (DingTalk, Feishu, etc.) are restored on restart.
    await ws.discover_per_user_channel_managers()
...
async def create_agent_config_watcher(ws: "Workspace", _):
    """Create agent config watcher if cron manager exists.
