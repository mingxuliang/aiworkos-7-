...
from ...agents.react_agent import AiWorkAgent
...
class AgentRunner(Runner):
    ...
    def refresh_state(self, task_tracker: Any | None = None) -> None:
        """Refresh mutable references after runner reuse across reload.

        Called by ServiceManager.reload_func when the runner is transferred
        to a new Workspace instance. Updates references that would otherwise
        point to the old (stopped) workspace.

        Args:
            task_tracker: New task tracker from the new workspace
        """
    ...
    async def query_handler(
        ...
        **kwargs,
        ...
        try:
            ...
            out_sender_id = request.user_id
            # JWT-authenticated user (set by per-user CM channels).
            # Used below for directory / memory / session isolation so
            # that platform sender_id (out_sender_id) is preserved in
            # chat records and last_dispatch for cron / heartbeat push.
            ...
            # Resolve per-user working directory, memory manager,
            # and session for shared agent user isolation.
            # Only scoped to JWT user_id — platform sender_id must not
            # create isolated directories.
            user_working_dir = None
            user_mm = self.memory_manager
            user_session = self.session
            if user_id and self._workspace is not None:
                user_working_dir = self._workspace.get_user_working_dir(
                    user_id,
                )
                user_mm = await self._workspace.get_memory_manager(
                    user_id,
                )
                user_session = self._workspace.get_user_session(user_id)

            env_context = build_env_context(
                session_id=session_id,
                user_id=out_sender_id,
                channel=channel,
                working_dir=(
                    str(user_working_dir)
                    if user_working_dir
                    else str(self.workspace_dir or WORKING_DIR)
                ),
            ...
            session_container_key = (
                f"{self.agent_id}:{out_sender_id}:{root_session_id}"
            )
            from ...security.sandbox.context import (
            ...
            if plan_notebook is not None:
                try:
                    _states = await user_session.get_session_state_dict(
                        session_id=session_id,
                        user_id=out_sender_id,
                    ...
                    if _agent_st and (
                        ...
                        await user_session.update_session_state(
                            session_id=session_id,
            ...
            try:
                await user_session.load_session_state(
                    session_id=session_id,
                    user_id=out_sender_id,
                    agent=agent,
                )
            ...
            if agent is not None and session_state_loaded:
                # Migrate legacy session file to user-scoped dir if needed
                user_session._migrate_legacy_file(session_id, out_sender_id)
                await user_session.save_session_state(
                    session_id=session_id,
                    user_id=out_sender_id,
                    agent=agent,
                )
    ...
    async def init_handler(self, *args, **kwargs):
        ...
        self.session = SafeJSONSession(save_dir=base_dir)

    async def shutdown_handler(self, *args, **kwargs):
        """
        Shutdown handler — called during final agent shutdown.

        Cleans up session and logs shutdown for debugging.
        The underlying Tokio runtime (from agentscope_runtime) is not
        explicitly shut down here because the upstream package does not
        expose a cleanup API. Runner reuse (reusable=True + stopped
        runner cache) is the primary mitigation for thread leaks.
        """
        ...
        if hasattr(self, "session") and self.session is not None:
            try:
                ...
                logger.debug(
                    "Error closing session for agent %s",
                    self.agent_id,
                    exc_info=True,
                )
        self._health = False
