...
async def get_session(
    ...
    When JWT authentication is active, returns a per-user session that
    stores files in users/{user_id}/sessions/.
    ...
    if jwt_user:
        return workspace.get_user_session(jwt_user)
...
async def create_chat(
    ...
    When ``out_sender_id`` is not provided, the server looks up existing
    chats with the same ``session_id`` and reuses their ``out_sender_id``.
    The ``user_id`` field is always set from the JWT identity.

    Args:
        request: Chat creation request body
        req: FastAPI request (for JWT identity)
        mgr: Chat manager dependency
    ...
    # Resolve out_sender_id: reuse from existing chats with the same
    # session_id when the client did not supply one.
    out_sender_id = request.out_sender_id
    if not out_sender_id:
        out_sender_id = await mgr.find_out_sender_id_by_session(
            request.session_id,
            request.channel,
        )

    # user_id is always the platform JWT identity
    jwt_user_id = await _get_jwt_user(req) or ""
...
async def batch_delete_chats(
    ...
    for chat_id in chat_ids:
        ...
        # Session key uses the platform sender_id for scoping.
        sender_id = chat_spec.out_sender_id or chat_spec.user_id
...
async def _check_chat_ownership(chat_spec: ChatSpec, request: Request) -> None:
    ...
    No-op when:
    - JWT authentication is not active (no user in request.state).
    - chat_spec.user_id is empty (legacy / edge-case chat with no owner;
      any authenticated user may manage it).
    ...
    if not jwt_user:
        return
    if not chat_spec.user_id:
        return
    if chat_spec.user_id != jwt_user:
        raise HTTPException(
            status_code=403,
...
async def get_chat(
    ...
    # Session scoping uses the platform sender_id.  Fall back to
    # user_id for legacy records that predate the out_sender_id field.
    sender_id = chat_spec.out_sender_id or chat_spec.user_id
