...
def _check_aiwork_not_running():
    """Check if AiWork is not running, exit if it is."""
    from ..config.utils import is_aiwork_running

    if is_aiwork_running():
        click.echo(
            "❌ AiWork is currently running. Please stop it first:",
            err=True,
        )
        click.echo("   aiwork shutdown", err=True)
        click.echo(
            "\n💡 Plugin operations are only allowed when AiWork is stopped.",
        )
        raise click.Abort()
...
def install(source: str, force: bool):
    ...
    Examples:
        aiwork plugin install examples/plugins/idealab-provider
        aiwork plugin install /path/to/plugin
        aiwork plugin install https://example.com/plugin.zip
    """
    from ..config.utils import get_plugins_dir

    # Check if AiWork is running
    ...
    click.echo("\n💡 Next steps:")
    click.echo("   1. Restart AiWork to load the plugin")
...
def uninstall(plugin_id: str):
    ...
    try:
        ...
        click.echo("💡 Restart AiWork to apply changes")
    except Exception as e:
