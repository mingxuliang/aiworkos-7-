...
class InstallInfo:
    """Information about the current AiWork installation."""
...
class RunningServiceInfo:
    """Detected AiWork service endpoint state."""
...
def _fetch_latest_version() -> str:
    """Fetch the latest published AiWork version from PyPI."""
    try:
        resp = httpx.get(
            _PYPI_JSON_URL,
            timeout=10.0,
        ...
        raise click.ClickException(
            f"Failed to fetch the latest AiWork version from PyPI: {exc}",
        ) from exc
    except json.JSONDecodeError as exc:
        raise click.ClickException(
            "Received an invalid response from PyPI when checking for the "
            f"latest AiWork version: {exc}",
        ) from exc
    version = str(data.get("info", {}).get("version", "")).strip()
    if not version:
        raise click.ClickException(
            "Unable to determine the latest AiWork version.",
        )
    return version
...
def _probe_service(base_url: str) -> RunningServiceInfo:
    """Probe a possible running AiWork HTTP service."""
    try:
        resp = httpx.get(
...
def _process_candidate_ports() -> list[int]:
    """Infer candidate local AiWork service ports from running processes."""
    ports: list[int] = []
...
def _detect_running_service(
    ...
    """Detect whether a AiWork HTTP service is currently running."""
    candidates: list[str] = []
    seen: set[str] = set()
    preferred_hosts: list[str] = []
...
def _running_service_display(running: RunningServiceInfo) -> str:
    ...
    if not running.base_url:
        return "a running AiWork service"
    version_suffix = f" (version {running.version})" if running.version else ""
    return f"AiWork service at {running.base_url}{version_suffix}"


def _confirm_force_shutdown(running: RunningServiceInfo) -> bool:
    """Ask whether `aiwork shutdown` should be used before updating."""
    click.echo("")
    ...
    click.secho(
        "Running `aiwork shutdown` will forcibly terminate the current "
        "AiWork backend/frontend processes.",
        fg="red",
        bold=True,
    )
    click.secho(
    ...
    return click.confirm(
        "Run `aiwork shutdown` now and continue with the update?",
        default=False,
    )
...
def _run_shutdown_for_update(
    ...
    """Run `aiwork shutdown` in the current environment before updating."""
    ...
    click.echo("")
    click.echo("Running `aiwork shutdown` before updating...")
    ...
    try:
        ...
        raise click.ClickException(
            "Failed to run `aiwork shutdown`: " f"{exc}",
        ) from exc
    ...
    if result.returncode != 0:
        raise click.ClickException(
            "`aiwork shutdown` failed. Please stop the running AiWork "
            "service manually before running `aiwork update`.",
        )
...
def run_update_worker(plan_path: str | Path) -> int:
    ...
    if return_code == 0:
        ...
        click.echo(
            "[aiwork] Please restart any running AiWork service "
...
def update_cmd(ctx: click.Context, yes: bool) -> None:
    """Upgrade AiWork in the current Python environment."""
    ...
    if version_check is False:
        click.echo("AiWork is already up to date.")
        return
    ...
    if running.is_running:
        if yes:
            raise click.ClickException(
                ...
                "Please stop it before running `aiwork update`, or rerun "
                "without `--yes` to confirm a forced `aiwork shutdown`.",
            )
        ...
        if running.is_running:
            raise click.ClickException(
                f"Detected {_running_service_display(running)} "
                "after `aiwork shutdown`. "
                "Please stop it manually before running `aiwork update`.",
            )

    if not yes and not click.confirm(
        f"Update AiWork to {latest_version} in the current environment?",
        default=True,
    ):
        click.echo("Cancelled.")
        return
    ...
    click.echo("")
    click.echo("Starting AiWork update...")
