...
def chats_group() -> None:
    ...
    \b
    Common examples:
      aiwork chats list                    # List all chats
      aiwork chats list --user-id alice    # Filter by user
      aiwork chats get <chat_id>           # View details
      aiwork chats create --session-id s1 --user-id u1
      aiwork chats delete <chat_id>        # Delete a chat
    """
...
def list_chats(
    ...
    Examples:
      aiwork chats list
      aiwork chats list --user-id alice
      aiwork chats list --channel discord
      aiwork chats list --user-id alice --channel discord
    """
...
def get_chat(
    ...
    \b
    CHAT_ID  Chat UUID, obtainable via `aiwork chats list`.

    \b
    Examples:
      aiwork chats get 823845fe-dd13-43c2-ab8b-d05870602fd8
    """
    base_url = resolve_base_url(ctx, base_url)
    with client(base_url) as c:
        headers = {"X-Agent-Id": agent_id}
...
def create_chat(
    ...
    Inline creation examples:
      aiwork chats create --session-id "discord:alice" \\
    ...
    JSON file creation example:
      aiwork chats create -f chat.json
    """
    base_url = resolve_base_url(ctx, base_url)
...
def update_chat(
    ...
    \b
    Examples:
      aiwork chats update <chat_id> --name "Renamed Chat"
    """
    base_url = resolve_base_url(ctx, base_url)
    headers = {"X-Agent-Id": agent_id}
    payload = {"name": name}
...
def delete_chat(
    ...
    \b
    Examples:
      aiwork chats delete 823845fe-dd13-43c2-ab8b-d05870602fd8
    """
    base_url = resolve_base_url(ctx, base_url)
    with client(base_url) as c:
        headers = {"X-Agent-Id": agent_id}
