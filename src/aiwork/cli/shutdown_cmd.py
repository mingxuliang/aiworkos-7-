...
def _find_frontend_dev_pids() -> set[int]:
    ...
    for pid, command in _process_table():
        ...
        if "vite" in lowered and console_dir in lowered:
            matches.add(pid)
            continue
        if "aiwork-console" in lowered and (
            "npm" in lowered
            or "pnpm" in lowered
            or "yarn" in lowered
            or "node" in lowered
...
def _find_desktop_wrapper_pids() -> set[int]:
    """Find `aiwork desktop` wrapper processes for this project."""
    matches: set[int] = set()
    patterns = (
        " -m aiwork desktop",
        " aiwork desktop",
...
def _find_windows_wrapper_ancestor_pids(pids: set[int]) -> set[int]:
    """Find AiWork wrapper/supervisor ancestors for Windows backend PIDs."""
...
@click.command(
    "shutdown",
    help="Force stop the running AiWork app processes.",
)
@click.option(
    "--port",
    default=None,
...
def shutdown_cmd(ctx: click.Context, port: Optional[int]) -> None:
    """Stop the running AiWork app processes.

    `aiwork app` only starts the backend process. The web console is normally
    ...
    if not all_targets:
        raise click.ClickException(
            "No running AiWork backend/frontend process was found.",
        )
