# -*- coding: utf-8 -*-
"""Periodic cleanup tasks for the presale template module.

Cleanup routine (runs every 30 minutes):
1. Expired PresaleUploadSession → abort MinIO multipart + mark aborted.
2. Soft-deleted PresaleTemplate → delete orphan MinIO objects, then
   hard-delete the DB row after a grace period (7 days).
"""
...
from .models import PresaleTemplate, PresaleUploadSession
...
# Soft-deleted templates: MinIO object is removed after this grace period.
# Within the grace period, an admin could manually restore the template
# by flipping is_deleted back to 0 (the MinIO object still exists).
...
async def _cleanup_expired_sessions(
    ...
    stmt = select(PresaleUploadSession).where(
        PresaleUploadSession.status == "uploading",
        PresaleUploadSession.expires_at < now,
    )
    ...
    if cleaned:
        await db.commit()
        logger.info(
            "Presale: cleaned up %d expired upload session(s)", cleaned,
        )
...
async def _cleanup_orphan_objects(
    ...
    """Delete MinIO objects for soft-deleted PresaleTemplates past the grace
    ...
    stmt = select(PresaleTemplate).where(
        PresaleTemplate.is_deleted == 1,  # noqa: E712
        PresaleTemplate.created_at < cutoff,
    ...
    for record in orphan_records:
        try:
            ...
            logger.debug(
                "Failed to delete orphan MinIO object %s (template id=%s)",
    ...
    if cleaned:
        ...
        await db.execute(
            sa_delete(PresaleTemplate).where(
                PresaleTemplate.id.in_(ids_to_delete),
            )
        )
        await db.commit()
        logger.info(
            "Presale: cleaned up %d orphan MinIO object(s) + hard-deleted DB rows",
            cleaned,
        )
...
async def run_presale_cleanup_loop(
    db_session_factory,
    ...
    2. Cleans up orphan MinIO objects for soft-deleted templates past the
       grace period (7 days).

    Intended to be launched via ``asyncio.create_task()`` during startup.
    """
    logger.info(
        "Presale template cleanup task started (interval=%ds)", interval,
    )
    while True:
        ...
        try:
            ...
            logger.error(
                "Presale template cleanup tick failed", exc_info=True,
            )
