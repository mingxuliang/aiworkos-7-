#!/usr/bin/env bash
# =============================================================================
# AIWork-OS 后端快速部署脚本（生产环境）
#
# 适用场景：仅修改后端代码后快速重新部署。流程：
#   1. 校验前置依赖（docker、.env）
#   2. 拉取后端最新代码
#   3. 构建后端镜像（跳过前端构建，复用已有 console-dist/）
#   4. 通过 docker compose 启动 / 重建服务
#   5. 等待健康检查并打印验证信息
#
# 用法：
#   bash deploy/deploy_backend.sh                          # 默认 IMAGE=aiwork:latest
#   IMAGE=aiwork:v1.0.0 bash deploy/deploy_backend.sh
#
# 环境变量：
...
if [[ ! -d "$REPO_ROOT/console-dist" ]]; then
    log_warn "未找到 console-dist/ 目录"
    log_warn "首次部署请先运行: bash deploy/deploy.sh"
    log_warn "或使用完整部署脚本构建前端"
    exit 1
fi
log_ok "前端构建产物: console-dist/ (将复用)"

# ---------------------------------------------------------------------------
# Step 2: 拉取后端代码并构建镜像
# ---------------------------------------------------------------------------
log_step "Step 2/4: 拉取后端代码并构建镜像 IMAGE=$IMAGE"

# 2.1 git pull 后端代码
log_info "正在拉取后端最新代码: $REPO_ROOT"
if (cd "$REPO_ROOT" && git pull --rebase --autostash); then
    log_ok "后端代码已更新"
else
    log_warn "后端 git pull 失败，继续部署当前代码"
fi

# 2.2 构建镜像（跳过后端构建，复用已有 console-dist/）
log_info "开始构建镜像（复用已有前端构建产物）..."
...
${C_BOLD}${C_GREEN}后端部署完成。${C_RESET}
  ...
  再次部署:   bash deploy/deploy_backend.sh
  完整部署:   bash deploy/deploy.sh
  前端部署:   bash deploy/deploy_frontend.sh
