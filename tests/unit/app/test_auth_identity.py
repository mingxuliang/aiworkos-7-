...
from aiwork.app.auth_identity import (
...
async def test_get_jwt_user_key_falls_back_to_jwt_sub(
    ...
    with patch(
        "aiwork.app.auth_jwt.middleware.JWTAuthMiddleware._extract_token",
        return_value="token",
    ), patch(
        "aiwork.app.auth_jwt.jwt_utils.decode_token",
        new=AsyncMock(return_value={"sub": "118", "username": "bob"}),
    ), patch(
        "aiwork.app.auth_jwt.redis_client.get_session_user_info",
...
def test_get_legacy_user_key_from_request_valid_token() -> None:
    ...
    with patch(
        ...
        "aiwork.app.auth_identity.verify_token",
        return_value="admin",
    ):
...
async def test_get_authenticated_user_key_jwt_mode(
    ...
    monkeypatch.setattr("aiwork.app.auth_identity.AUTH_MODE", "jwt")
    request = MagicMock()
    request.state.user_id = "117"
...
async def test_get_authenticated_user_key_legacy_mode(
    ...
    monkeypatch.setattr("aiwork.app.auth_identity.AUTH_MODE", "legacy")
    request = MagicMock()
    with patch("aiwork.app.auth_identity.is_auth_enabled", return_value=True), patch(
        "aiwork.app.auth_identity.get_legacy_user_key_from_request",
        return_value="admin",
    ):
